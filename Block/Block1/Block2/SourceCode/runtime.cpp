...
static int32_t latching_incr_int(volatile int32_t *where) {
    while (1) { //线程安全
        ...
        //如果flags含有 BLOCK_REFCOUNT_MASK 证明其引用计数达到最大，直接返回
        if ((old_value & BLOCK_REFCOUNT_MASK) == BLOCK_REFCOUNT_MASK) {
            return BLOCK_REFCOUNT_MASK;
        }
        //原子性判断其值当前是否被其他线程改动，如果被改动就进入下一次循环直到改动结束后赋值。
        //OSAtomicCompareAndSwapInt的作用就是在where取值与old_value相同时，将old_value+2赋给where。
        //注:Block的引用计数以flags的后16位代表，以 2 为单位，每次递增2，1 被 BLOCK_DEALLOCATING 正在释放占用。
...
void *_Block_copy(const void *arg) {
    ...
    if (aBlock->flags & BLOCK_NEEDS_FREE) { //该Block为堆Block
        // latches on high
        //对其引用计数递增
        latching_incr_int(&aBlock->flags);
        return aBlock;
    }
    else if (aBlock->flags & BLOCK_IS_GLOBAL) { //Block为全局Block
        return aBlock;
    }
    else { //栈Block
...
#endif
        // reset refcount
        result->flags &= ~(BLOCK_REFCOUNT_MASK|BLOCK_DEALLOCATING);    // XXX not needed 引用计数位置零
        result->flags |= BLOCK_NEEDS_FREE | 2;  // logical refcount 1 将Block标识为堆Block并将其引用计数置为2
        _Block_call_copy_helper(result, aBlock); //如果有copy_dispose助手，就执行Block的保存的copy函数
