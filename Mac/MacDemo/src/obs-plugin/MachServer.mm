...
- (void)run {
    ...
    /** It's a bummer this is deprecated. The replacement, NSXPCConnection, seems to require an assistant process that lives inside the .app bundle. This would be more modern, but adds complexity and I think makes it impossible to just run the `obs` binary from the commandline. So let's stick with NSMachBootstrapServer at least until it fully goes away. At that point we can decide between NSXPCConnection and using the CoreFoundation versions of these APIs (which are, interestingly, not deprecated) */
