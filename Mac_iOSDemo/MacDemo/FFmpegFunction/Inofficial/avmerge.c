...
int avmerge(char *src_file1, char *src_file2, char *out_file)
{
    ...
    //open first file
    if((err_code = avformat_open_input(&inputFormatContext1, src_file1, 0, 0)) < 0 ){
    ...
    if((err_code = avformat_find_stream_info(inputFormatContext1, 0)) <0){
        av_strerror(err_code, errors, ERROR_STR_SIZE);
        av_log(NULL, AV_LOG_ERROR,
    ...
    av_dump_format(inputFormatContext1, 0, src_file1, 0);
    
    //open second file
    if((err_code = avformat_open_input(&inputFormatContext2, src_file2, 0, 0)) < 0 ){
        av_strerror(err_code, errors, ERROR_STR_SIZE);
        av_log(NULL, AV_LOG_ERROR,
    ...
    if((err_code = avformat_find_stream_info(inputFormatContext2, 0)) <0){
        av_strerror(err_code, errors, ERROR_STR_SIZE);
        av_log(NULL, AV_LOG_ERROR,
               "Failed to retrieve input stream info, %s, %d(%s) \n",
               src_file2, err_code, errors);
    ...
    av_dump_format(inputFormatContext2, 0, src_file2, 0);
    
    //create out context
    if((err_code = avformat_alloc_output_context2(&outputFormatContext, NULL, NULL, out_file)) < 0 ){
        av_strerror(err_code, errors, ERROR_STR_SIZE);
        av_log(NULL, AV_LOG_ERROR,
    ...
    if(inputFormatContext1->nb_streams == 1){
        in_stream1 = inputFormatContext1->streams[0];
        stream1 = 1;
        ...
        out_stream1 = avformat_new_stream(outputFormatContext, NULL);
        if(!out_stream1){
    ...
    if(inputFormatContext2->nb_streams == 1){
        in_stream2 = inputFormatContext2->streams[0];
        stream2 = 1;
        ...
        out_stream2 = avformat_new_stream(outputFormatContext, NULL);
        if(!out_stream2){
            av_log(NULL, AV_LOG_ERROR, "Failed to alloc out stream!\n");
            goto __FAIL;
        }
    ...
    av_dump_format(outputFormatContext, 0, out_file, 1);
    
    //open out file
    if(!(outputFormat->flags & AVFMT_NOFILE)){
        if((err_code = avio_open(&outputFormatContext->pb, out_file, AVIO_FLAG_WRITE))<0){
            av_strerror(err_code, errors, ERROR_STR_SIZE);
    ...
    //write media header
    if((err_code = avformat_write_header(outputFormatContext, NULL)) < 0){
        av_strerror(err_code, errors, ERROR_STR_SIZE);
        av_log(NULL, AV_LOG_ERROR, "Error occurred when writing media header!\n");
        goto __FAIL;
    }
    ...
    while ( stream1 || stream2 ) {
        ...
        if (stream1 &&
            (!stream2 || av_compare_ts(cur_pts1, in_stream1->time_base, cur_pts2, in_stream2->time_base) <= 0)) {
            ret = av_read_frame(inputFormatContext1, &pkt);
            if(ret < 0){
                stream1 = 0;
                continue;
            }
            ...
            stream1 = !av_interleaved_write_frame(outputFormatContext, &pkt);
            //stream1 = !av_write_frame(ofmt_ctx, &pkt);
        } else if(stream2){
            ret = av_read_frame(inputFormatContext2, &pkt);
            if(ret < 0 ){
                stream2 = 0;
                continue;
            }
            ...
            stream2 = !av_interleaved_write_frame(outputFormatContext, &pkt);
        }
    ...
    //write media tailer
    if((err_code = av_write_trailer(outputFormatContext)) < 0){
        av_strerror(err_code, errors, ERROR_STR_SIZE);
...
__FAIL:
    ...
    if(inputFormatContext1){
        avformat_close_input(&inputFormatContext1);
    }
    
    if(inputFormatContext2){
