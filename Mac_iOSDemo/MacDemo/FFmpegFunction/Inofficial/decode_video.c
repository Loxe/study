...
int decode_video(const char *filename, const char *outfilename)
{
    ...
    while (av_read_frame(fmt_ctx, &avpkt) >= 0) {
        ...
        if(avpkt.stream_index == stream_index){
            if (decode_write_frame(outfilename, codecContext, img_convert_ctx, frame, &frame_count, &avpkt, 0) < 0)
                return -1;
        }
    ...
    avpkt.data = NULL;
    avpkt.size = 0;
    decode_write_frame(outfilename, codecContext, img_convert_ctx, frame, &frame_count, &avpkt, 1);
