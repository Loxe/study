...
/**
 处理StartCode和SPS, PPS
 码流格式: Annex-B, AVCC(H.264)与HVCC(H.265)
...
int h264_mp4toannexb(AVFormatContext *fmt_ctx, AVPacket *in, FILE *dst_fd)
{
    ...
    do {
        ...
        // 一个packet里可能有多个帧, 前4个字节表示第一帧的大小, 这里是取出前4个字节数据
        for (nal_size = 0, i = 0; i<4/*s->length_size*/; i++)
            nal_size = (nal_size << 8) | buf[i];
        
        buf += 4; /*s->length_size;*/
        unit_type = *buf & 0x1f; // 后5位表示nalu(Network Abstract Layer unit)单元类型
        ...
        if (/*s->new_idr && */unit_type == 5 /*&& !s->idr_sps_seen && !s->idr_pps_seen*/) { // 关键帧
        ...
        len = (int)fwrite( out->data, 1, out->size, dst_fd); // 写入缓存
        if(len != out->size){
            av_log(NULL, AV_LOG_DEBUG, "warning, length of writed data isn't equal pkt.size(%d, %d)\n",
                   len,
                   out->size);
        }
        fflush(dst_fd); // 真正写入文件
